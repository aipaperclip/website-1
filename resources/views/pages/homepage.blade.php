                            ...
                            <div class="row fs-0">
                                @foreach($applications as $application)
                                    <button class="col-md-3 col-xs-4 inline-block-top single-application">
                                        <figure class="wrapper" @if($application->media) data-image="{{URL::asset('assets/uploads/'.$application->media->name) }}" data-image-alt="@if($application->media->alt){{$application->media->alt}}@endif" @endif @if($application->popup_logo) data-popup-logo="{{URL::asset('assets/uploads/'.$application->popup_logo->name) }}" data-popup-logo-alt="@if($application->popup_logo->alt){{$application->popup_logo->alt}}@endif" @endif @if($application->text) data-description="{{ json_encode($application->text) }}" @endif  @if($application->slug == 'blog-intro') data-articles="{{json_encode($latest_blog_articles)}}" @endif itemscope="" data-slug="{{$application->slug}}" itemtype="http://schema.org/ImageObject">
                                            @if($application->logo)
